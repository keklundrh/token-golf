...
async def get_global_leaderboard(
    ...
    leaderboard_data = await scoring_service.get_global_leaderboard(
        limit=limit + offset  # Get more to handle offset
    )

    # Extract completed and in_progress lists from the returned dict
    # Filter completed only if requested, otherwise combine both lists
    if completed_only:
        leaderboard = leaderboard_data.get("completed", [])
    else:
        # Combine completed and in_progress entries
        leaderboard = leaderboard_data.get("completed", []) + leaderboard_data.get("in_progress", [])

    # Apply offset and limit
    total_entries = len(leaderboard)
    leaderboard = leaderboard[offset : offset + limit]

    # Build response entries (usernames already included in the scoring service response)
    entries = [
        LeaderboardEntry(
            rank=offset + idx + 1,
